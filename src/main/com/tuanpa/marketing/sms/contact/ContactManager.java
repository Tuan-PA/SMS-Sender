		...
		for (File file : listOfFiles) {
			if (file.isFile()) {
				...
				while ((temp = br.readLine()) != null) {
					...
					SMSLogger.getInstance().log("ListSend" + contact.getCarrier(), contact.getPhone());
				}
				br.close();
				// Save number contact each carrier into log.
				SMSLogger.getInstance().log("Statistic", "Viettel : " + noViettel );
				SMSLogger.getInstance().log("Statistic", "Mobifone : " + noMobifone );
				SMSLogger.getInstance().log("Statistic", "Vinaphone : " + noVinaphone );
				SMSLogger.getInstance().log("Statistic", "Unknow : " + noUnknow );
			}
		}
