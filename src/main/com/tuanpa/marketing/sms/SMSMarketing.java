...
public class SMSMarketing {
	...
	public boolean init() throws Exception{
		boolean isInited = autoDetectDevice() 
				&& readMessageFromFile()
				&& readContactFromFile();
	...
	public void registerGateway() throws InterruptedException {
		...
		for (int i = 0; i < ComPortDetect.lPort.size(); i++) {
			CommPortIdentifier cpi = ComPortDetect.lPort.get(i);
			Modem modem = new Modem(cpi.getName(), cpi.getName(), 9600, "0000",
					"0000", msisdn, "");
	...
	public void unregisterGateway() throws InterruptedException{
		for (String gatewayId : Service.getInstance().getGatewayIDs()) {
			AbstractGateway gate = Service.getInstance().getGatewayById(gatewayId);
	...
	/**
	 * 
	 * @param spm Number sms will send per minute
	 * @throws Exception 
	 */
	public void sendMessage(int spm) throws Exception{
		long timeSleep = 60000 / Service.getInstance().getGatewayIDs().size();
		...
		while (!ContactManager.isEnd()) {
			...
			if (contact.getCarrier() == carrier){
				...
				obMsg.setRequestDeliveryReport(true);
	...
	public String getStatistic(){
		String statistic = "\n";
		statistic += "\n Time Start: " + Service.getInstance().getStatistics().getStartTime().toString();
		statistic += "\n Total Failed: " + Service.getInstance().getStatistics().getTotalFailed();
		statistic += "\n Total Received: " + Service.getInstance().getStatistics().getTotalReceived();
		statistic += "\n Total Sent: " + Service.getInstance().getStatistics().getTotalSent();
		return statistic;
	}
}
